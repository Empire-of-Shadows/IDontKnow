...
def get_logger(
    ...
    # IMPORTANT: Disable propagation to prevent duplicate messages
    # We'll handle email notifications through our own handlers
    ...
    # Add any global handlers that were registered before this logger was created
    if hasattr(manager, 'global_handlers'):
        for global_handler in manager.global_handlers:
...
def setup_application_logging(
    ...
    if enable_performance_logging:
        _ = get_logger(
            ...
            level=logging.DEBUG,
            log_dir=log_dir,
            console_output=True,  # Performance logs go to file only
...
def add_global_handler(handler: logging.Handler):
    """
    Add a handler to all existing loggers managed by LoggerManager.
    Useful for adding email handlers or other global handlers.
    """
    manager = LoggerManager()

    # Add to root logger with proper level
    root_logger = logging.getLogger()
    root_logger.addHandler(handler)
    root_logger.setLevel(logging.DEBUG)  # Ensure root accepts all levels

    # Add to all managed loggers
    for logger_name, logger in manager.loggers.items():
        logger.addHandler(handler)
        print(f"Added global handler to logger: {logger_name}")

    # Store the handler in manager for future loggers
    if not hasattr(manager, 'global_handlers'):
        manager.global_handlers = []
    manager.global_handlers.append(handler)